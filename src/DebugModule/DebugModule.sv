module DebugModule #(
    parameter int unsigned TIMEOUT_TICKS = 1000000
) (
    input logic clk,
    input logic arstn,
  ...
  always_comb begin
    ...
    if (prog_mem_out_payload_we) begin
      ...
      out_payload_we = 1'b1;
    end else if (data_mem_out_payload_we) begin
      out_payload = data_mem_out_payload;
      out_payload_addr = data_mem_out_payload_addr;
      out_payload_we = 1'b1;
    end else if (state_out_payload_we) begin
    ...
    end else if (data_mem_in_data_re) begin
      inp_packet_data_addr = data_mem_in_data_addr;
      inp_packet_data_re   = 1'b1;
    end
  end
  ...
  localparam logic [7:0] CMD_HALT = 8'h00;
  localparam logic [7:0] CMD_RESUME = 8'h01;
  localparam logic [7:0] CMD_READ_PROG = 8'h02;
  localparam logic [7:0] CMD_WRITE_PROG = 8'h03;
  localparam logic [7:0] CMD_READ_DATA = 8'h04;
  localparam logic [7:0] CMD_WRITE_DATA = 8'h05;
  localparam logic [7:0] CMD_RESET = 8'h06;
  localparam logic [7:0] CMD_GET_STATE = 8'h07;

  localparam logic [7:0] STATUS_OK = 8'h00;
  localparam logic [7:0] STATUS_ERROR_PARITY = 8'hFF;
  localparam logic [7:0] STATUS_ERROR_ARGS = 8'hFE;
  localparam logic [7:0] STATUS_ERROR_CMD = 8'hFD;
        ...
        if (inp_packet_valid) begin
          if (inp_packet_error) begin
            ...
            case (inp_packet_cmd)
              ...
              CMD_READ_PROG: begin
                if (inp_packet_len >= 8'h05) next_state = S_CMD_READ_PROG;
                else next_state = S_ERROR_ARGS;
              end
              CMD_WRITE_PROG: begin
                if (inp_packet_len >= 8'h04) next_state = S_CMD_WRITE_PROG;
                else next_state = S_ERROR_ARGS;
              end
              CMD_READ_DATA: begin
                if (inp_packet_len >= 8'h05) next_state = S_CMD_READ_DATA;
                else next_state = S_ERROR_ARGS;
              end
              CMD_WRITE_DATA: begin
                if (inp_packet_len >= 8'h04) next_state = S_CMD_WRITE_DATA;
                else next_state = S_ERROR_ARGS;
              end
  ...
  always_ff @(posedge clk or negedge arstn) begin
    if (~arstn) begin
      ...
      if (input_valid) begin
        timeout_counter <= '0;
      end else if (cur_state == S_WAIT_TIMEOUT) begin
