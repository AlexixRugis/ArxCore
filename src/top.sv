module top (
  ...
  RamPort rom_port_b_inst (
      .clk(clk_50_mhz),
      .arstn(arstn),
      .clk_en(1'b1),
      .addr(rom_wr_port_addr),
      .write_data(rom_wr_port_write_data),
      .wr_en(rom_wr_port_we),
      .wr_mask(rom_wr_port_mask),
      .req(rom_wr_port_req),
      .data(rom_wr_port_data),
      .ack(rom_wr_port_ack),
      .ram_addr(port_b_addr),
      .ram_write_data(port_b_write_data),
      .ram_wr_en(port_b_we),
      .ram_byte_en(port_b_mask),
      .ram_data(port_b_read_data)
  );
  ...
  Peripherals periph (
      ...
      .arstn(arstn),
      .address(periph_addr),
      .data(periph_write_data),
      .write_en(periph_we),
      .write_mask(periph_mask),
      .q(periph_val),
      .out_1(out_1)
  );
  ...
  SevenSegmentInd ind_0 (
      .digit(digit[3:0]),
      .out  (hex_0)
  );
  SevenSegmentInd ind_1 (
      .digit(digit[7:4]),
      .out  (hex_1)
  );
  SevenSegmentInd ind_2 (
      .digit(digit[11:8]),
      .out  (hex_2)
  );
  SevenSegmentInd ind_3 (
      .digit(digit[15:12]),
      .out  (hex_3)
  );
