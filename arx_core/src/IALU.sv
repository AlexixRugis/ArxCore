...
#(
  ...
  always_comb begin
    case (opcode)
      IALU_LTU, IALU_GEU, IALU_SLTU: is_op_signed = 1'b0;
  ...
  localparam int unsigned SHIFT_WIDTH = $clog2(XLEN);
  ...
  always_comb begin
    case (opcode)
      IALU_ADD, IALU_SUB: m_res = sumsub_res;

      IALU_AND, IALU_XOR, IALU_OR: m_res = bitop_res;

      IALU_SLL, IALU_SRL, IALU_SRA: m_res = shift_res;

      IALU_SLTU, IALU_SLT: m_res = {{(XLEN - 1) {1'b0}}, lt_res};
  ...
  always_comb begin
    case (opcode)
      ...
      IALU_LTU, IALU_LT: m_branch_en = lt_res;
      IALU_GEU, IALU_GE: m_branch_en = ~lt_res;
