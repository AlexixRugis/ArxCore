...
module IMDU_tb;
  ...
  typedef struct packed {logic [XLEN-1:0] expected_res;} in_data_t;

  typedef struct packed {logic [XLEN-1:0] res;} out_data_t;

  mailbox #(in_data_t)  mb_in = new();
  mailbox #(out_data_t) mb_out = new();

  task send_transaction(input logic [XLEN-1:0] arg_1, input logic [XLEN-1:0] arg_2,
                        input mdu_op_e opcode, input logic [XLEN-1:0] expected_output);
    in_data_t in_data;
  ...
  function logic [31:0] div_signed(input logic signed [31:0] a, input logic signed [31:0] b);
    if (b == 0) return 32'hFFFFFFFF;  // Division by zero returns -1
  ...
  function logic [31:0] rem_signed(input logic signed [31:0] a, input logic signed [31:0] b);
    if (b == 0) return a;  // Remainder by zero returns dividend
    else if (a == 32'h80000000 && b == -1) return 0;  // Overflow case: -2^31 % -1 = 0
    else return a % b;
  endfunction
