...
module MemStage_tb;
  ...
  function logic [ADDR_WIDTH-1:0] get_aligned_addr(ls_type_e op_type);
    logic [1:0] offset;
    case (op_type)
  ...
  function logic [XLEN-1:0] extract_mem_data(logic [XLEN-1:0] mem_word, logic [ADDR_WIDTH-1:0] addr,
                                             ls_type_e op_type);
    logic [ 1:0] offset;
    logic [15:0] half;
    logic [ 7:0] byte_;
  ...
  task check_output();
    ...
    for (int i = 0; i < TRANSACTION_COUNT; i++) begin
      ...
      if (in_data.alu_result !== out_data.alu_result) begin
        $error("Alu result error! Expected: %h, Actual: %h", in_data.alu_result,
               out_data.alu_result);
      end
      ...
      if (in_data.reg_write !== out_data.reg_write) begin
        $error("reg_write mismatch! Expected: %h, Actual: %h", in_data.reg_write,
               out_data.reg_write);
      end

      if (in_data.mem_to_reg !== out_data.mem_to_reg) begin
        $error("mem_to_reg mismatch! Expected: %h, Actual: %h", in_data.mem_to_reg,
               out_data.mem_to_reg);
      end

      if (in_data.mem_op) begin
        if (mem_op.addr !== in_data.alu_result[XLEN-1:0] & 0'hFFFF_FFFC) begin
          $error("Memory read addr mismatch! Expected: %h, Actual: %h", in_data.alu_result,
                 mem_op.addr);
        end
        ...
        if (in_data.mem_op_type == STORE_WORD || in_data.mem_op_type == STORE_HALFWORD
          ...
          case (in_data.mem_op_type)
            ...
            STORE_HALFWORD: begin
              ...
              if (mem_op.write_mask !== expected_mask) begin
                $error("STORE_HALFWORD: Wrong mask! Expected %b, Got %b", expected_mask,
                       mem_op.write_mask);
              end
            end
            STORE_BYTE: begin
              ...
              if (mem_op.write_mask !== expected_mask) begin
                $error("STORE_BYTE: Wrong mask! Expected %b, Got %b", expected_mask,
                       mem_op.write_mask);
              end
            end
          endcase
          ...
          case (in_data.mem_op_type)
            STORE_WORD: begin
              if (mem_op.value !== in_data.rs2_value) begin
                $error("STORE WORD: Mem write value mismatch! Expected: %h, Actual: %h",
                       in_data.rs2_value, mem_op.addr);
              end
            end
            STORE_HALFWORD: begin
              if (in_data.alu_result[1] === 1'b0) begin
                if (mem_op.value[15:0] !== in_data.rs2_value[15:0]) begin
                  $error("STORE HALFWORD: Mem write value mismatch! Expected: %h, Actual: %h",
                         in_data.rs2_value, mem_op.addr);
                end
              end else begin
              ...
              case (in_data.alu_result[1:0])
                2'b00: begin
                  if (mem_op.value[7:0] !== in_data.rs2_value[7:0]) begin
                    $error("STORE BYTE: Mem write value mismatch! Expected: %h, Actual: %h",
                           in_data.rs2_value, mem_op.addr);
                  end
                end
                2'b01: begin
          ...
          if (extract_mem_data(
                  mem_op.value, in_data.alu_result, in_data.mem_op_type
              ) !== out_data.mem_value) begin
            $error("Mem read value mismatch! Expected: %h, Actual: %h", mem_op.value,
                   out_data.mem_value);
          end
        end
      end

      $display("[CHECK] Transaction %0d: ALU=%h, MEM=%h, RD=%0d", i, out_data.alu_result,
               out_data.mem_value, out_data.rd);
    end
  endtask
