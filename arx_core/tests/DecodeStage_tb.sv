...
module DecodeStage_tb;
  ...
  branch_type_e                  pc_addr_type_out;
  logic                          pc_jump_en_out;

  logic                          reg_write_out;
  logic                          mem_to_reg_out;
  logic                          mem_op_out;
  ls_type_e                      mem_op_type_out;

  alu_op_e                       alu_op_out;
  alu_src1_e                     alu_src_1_out;
  alu_src2_e                     alu_src_2_out;
  ...
  typedef struct packed {
    ...
    branch_type_e exp_pc_addr_type;
    ...
    logic exp_mem_op;
    ls_type_e exp_mem_op_type;

    alu_op_e   exp_alu_op;
    alu_src1_e exp_alu_op1;
    alu_src2_e exp_alu_op2;
  } in_data_t;
  ...
  typedef struct packed {
    ...
    alu_op_e alu_op;
    alu_src1_e alu_src_1;
    alu_src2_e alu_src_2;
  } out_data_t;
  ...
  task check_output();
    ...
    for (int i = 0; i < TRANSACTION_COUNT; i++) begin
      ...
      $display("INSN: %s opcode: %b funct3: %b funct7: %b", in_data.insn_class.name(),
               in_data.insn[6:0], in_data.insn[14:12], in_data.insn[31:25]);
      ...
      if (in_data.exp_reg_write !== out_data.reg_write) begin
        $error("reg_write mismatch! Expected: %h, Actual: %h", in_data.exp_reg_write,
               out_data.reg_write);
      ...
      if (in_data.exp_mem_to_reg !== out_data.mem_to_reg) begin
        $error("mem_to_reg mismatch! Expected: %h, Actual: %h", in_data.exp_mem_to_reg,
               out_data.mem_to_reg);
      end
