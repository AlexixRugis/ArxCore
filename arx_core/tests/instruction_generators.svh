...
function automatic logic [4:0] random_reg(input logic exclude_zero = 1);
  logic [4:0] regr;
  if (exclude_zero) regr = $urandom_range(1, 31);
  else regr = $urandom_range(0, 31);
  return regr;
endfunction
...
function automatic logic [31:0] gen_r_type(input logic [4:0] rd, input logic [4:0] rs1,
                                           input logic [4:0] rs2, input int op_idx);
  return {R_INSTRUCTIONS[op_idx].funct7, rs2, rs1, R_INSTRUCTIONS[op_idx].funct3, rd, 7'b0110011};
endfunction
...
function automatic logic [31:0] gen_i_type_arith(input logic [4:0] rd, input logic [4:0] rs1,
                                                 input int imm, input int op_idx);
  ...
  return {funct7, imm12[4:0], rs1, I_INSTRUCTIONS[op_idx].funct3, rd, 7'b0010011};
endfunction

// ============== Load instructions ==============
const
ls_type_e
LOAD_TYPES[] = '{LOAD_BYTE, LOAD_HALFWORD, LOAD_WORD, LOAD_BYTE_UNSIGNED, LOAD_HALFWORD_UNSIGNED};

function automatic logic [31:0] gen_load(input logic [4:0] rd, input logic [4:0] rs1,
                                         input int offset, input int load_idx);
...
// ============== Store instructions ==============
const ls_type_e STORE_TYPES[] = '{STORE_BYTE, STORE_HALFWORD, STORE_WORD};

function automatic logic [31:0] gen_store(input logic [4:0] rs1, input logic [4:0] rs2,
                                          input int offset, input int store_idx);
  logic [11:0] offset12 = offset[11:0];
  logic [ 2:0] funct3;
...
function automatic logic [31:0] gen_branch(input logic [4:0] rs1, input logic [4:0] rs2,
                                           input int offset, input int br_idx);
...
// ============== JAL (J-type) ==============
function automatic logic [31:0] gen_jal(input logic [4:0] rd, input int offset);
...
// ============== JALR (I-type) ==============
function automatic logic [31:0] gen_jalr(input logic [4:0] rd, input logic [4:0] rs1,
                                         input int offset);
  logic [11:0] offset12 = offset[11:0];
...
// ============== LUI (U-type) ==============
function automatic logic [31:0] gen_lui(input logic [4:0] rd, input int imm);
...
// ============== AUIPC (U-type) ==============
function automatic logic [31:0] gen_auipc(input logic [4:0] rd, input int imm);
  logic [19:0] imm20 = imm[31:12];
...
function automatic logic [31:0] gen_random_instruction(
    output insn_class_t insn_class, output logic [4:0] rd, output logic [4:0] rs1,
    output logic [4:0] rs2, output int imm);
  ...
  case (insn_type)
    ...
    1: begin
      ...
      else imm = sign_extend($urandom(), 12);
